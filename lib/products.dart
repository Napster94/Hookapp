...
class _ProductsPageState extends State<ProductsPage> {
  ...
  Future<void> _loadProducts() async {
    final box = await Hive.openBox<Product>('productsBox2');
    if(box.isEmpty) {
      products = [
        Product(name: 'Hoses', image: "assets/pipe.jpg", price: 0.26),
      ];
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (isLoading) const LoadingAllpages(),
        ],
      ),
    );
  }
                                          ...
                                          if (_controllers[index].isCompleted)
                                            IconButton(
                                              icon: _counters[index] == 1
                                                  ? Icon(CupertinoIcons.delete, color: Colors.yellow.shade600)
                                                  : Icon(Icons.remove, color: Colors.yellow.shade600),
                                              onPressed: () {
